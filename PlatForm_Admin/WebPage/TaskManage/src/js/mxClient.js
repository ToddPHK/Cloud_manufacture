...
navigator.userAgent.indexOf('Camino/1')>=0||navigator.userAgent.indexOf('Epiphany/2')>=0||navigator.userAgent.indexOf('Iceape/1')>=0||navigator.userAgent.indexOf('Galeon/2')>=0||navigator.userAgent.indexOf('Opera/9')>=0||navigator.userAgent.indexOf('AppleWebKit/5')>=0,
...
{
var result=null;
if(mxUtils.getFunctionName(array.constructor))
{
...
while(node!=null)
{
var clone=(mxClient.IS_IE)?node.cloneNode(true):
...
redirectMouseEvents:function(element,graph,cell,index)
{
mxEvent.addListener(element,'mousedown',function(evt)
{
...
{
graph.dispatchGraphEvent('mousemove',evt,cell,index);
});
mxEvent.addListener(element,'mouseup',function(evt)
{
graph.dispatchGraphEvent('mouseup',evt,cell,index);
});
},
...
if(mxClient.IS_NS)
{
mxEvent.addListener(window,'DOMMouseScroll',wheelHandler);
}
else
{
...
if(this.isDashed&&this.strokeNode==null)
{
...
mxShape.prototype.ENABLE_FOREIGNOBJECT=false;
...
var endSize=mxUtils.getValue(this.style,mxConstants.STYLE_ENDTSIZE,mxConstants.DEFAULT_MARKERSIZE)*this.scale;
...
if(startSize<6)
{
startWidth='narrow',startLength='short';
}
...
if(endSize<6)
{
endWidth='narrow',endLength='short';
}
else if(endSize>10)
{
endWidth='wide';
...
{
return!this.graph.getModel().isEdge(edge)||!this.graph.isCellVisible(edge)||this.graph.getModel().getTerminal(edge,true)==null||this.graph.getModel().getTerminal(edge,false)==null;
};
...
var idx=Math.max(0,i-((i>idx)?1:0));
...
{
return mxGraphLayout.prototype.isVertexIgnored.apply(this,arguments)||this.graph.getConnections(cell).length==0;
};
...
var childCount=this.graph.getModel().getChildCount(parent);
for(var i=0;i<childCount;i++)
{
var child=model.getChildAt(parent,i);
if(!this.isEdgeIgnored(child))
{
var src=this.graph.getView().getVisibleTerminal(child,true);
var trg=this.graph.getView().getVisibleTerminal(child,false);
...
if(state!=null)
{
var edgeStyle=(source!=null&&source==target)?state.style[mxConstants.STYLE_LOOP]||this.graph.defaultLoopStyle:
...
{
if(self.isContainerEvent(evt)&&(!mxClient.IS_IE||!self.isScrollEvent(evt)))
{
graph.dispatchGraphEvent('mousedown',evt);
}
});
...
mxGraph.prototype.edgeLabelMovable=true;
mxGraph.prototype.vertexLabelMovable=false;
...
mxGraph.prototype.htmlLabels=mxClient.IS_MAC;
...
this.dispatchEvent('afterMove',this,cells);
}
return clones;
};
...
{
this.setCollapsedState(cells,true);
};
mxGraph.prototype.expand=function(cells)
{
this.setCollapsedState(cells,false);
};
mxGraph.prototype.setCollapsedState=function(cells,collapsed)
{
cells=cells||this.getSelectionCells();
if(cells!=null&&cells.length>0)
{
...
{
return!this.isLocked(cell)&&((this.model.isEdge(cell)&&this.edgeLabelMovable)||(this.model.isVertex(cell)&&this.vertexLabelMovable));
};
...
if(!this.isEditing()&&(mxClient.IS_OP||mxClient.IS_SF||evt.target!=this.container))
{
...
if(this.graph.isMovable(cell)&&((!model.isEdge(cell)||this.graph.getSelectionCount()>1||(geo.points!=null&&geo.points.length>0)||model.getTerminal(cell,true)==null||model.getTerminal(cell,false)==null)||this.graph.allowDanglingEdges||(evt.ctrlKey&&this.graph.isCloneable())))
{
this.startX=evt.clientX;
this.startY=evt.clientY;
...
var clone=evt.ctrlKey&&graph.isCloneable()&&this.cloneEnabled;
...
var scale=graph.getView().scale;
var clone=evt.ctrlKey&&graph.isCloneable()&&this.cloneEnabled;
var dx=graph.snap((evt.clientX-this.startX)/scale);
var dy=graph.snap((evt.clientY-this.startY)/scale);
if(evt.shiftKey)
{
if(Math.abs(dx)>Math.abs(dy))
{
...
mxPanningHandler.prototype.submenuImage=mxClient.imageBasePath+'submenu.gif';
mxPanningHandler.prototype.graph=null;
mxPanningHandler.prototype.factoryMethod=true;
mxPanningHandler.prototype.useShiftKey=true;
mxPanningHandler.prototype.usePopupTrigger=true;
mxPanningHandler.prototype.useLeftButtonForPanning=false;
mxPanningHandler.prototype.useLeftButtonForPopup=false;
mxPanningHandler.prototype.ignoreCell=false;
mxPanningHandler.prototype.useGrid=false;
mxPanningHandler.prototype.selectOnPopup=true;
mxPanningHandler.prototype.enabled=true;
mxPanningHandler.prototype.panningEnabled=true;
mxPanningHandler.prototype.itemCount=0;
...
mxPanningHandler.prototype.isEnabled=function()
{
return this.enabled;
};
mxPanningHandler.prototype.setEnabled=function(enabled)
{
this.enabled=enabled;
};
mxPanningHandler.prototype.isPopupTrigger=function(evt,cell)
{
...
mxPanningHandler.prototype.addItem=function(title,image,funct,parent)
{
parent=parent||this;
this.itemCount++;
var tr=document.createElement('tr');
...
mxPanningHandler.prototype.createSubmenu=function(parent)
{
...
mxPanningHandler.prototype.showSubmenu=function(parent,row)
{
if(row.div!=null)
{
...
mxPanningHandler.prototype.addSeparator=function(parent)
{
parent=parent||this;
var tr=document.createElement('tr');
var col1=document.createElement('td');
...
mxPanningHandler.prototype.popup=function(evt,cell)
{
...
mxPanningHandler.prototype.isMenuShowing=function()
{
return this.div.parentNode==document.body;
};
mxPanningHandler.prototype.showMenu=function()
{
...
mxPanningHandler.prototype.showShadow=function()
{
...
mxPanningHandler.prototype.hideMenu=function()
{
if(this.div!=null)
{
...
mxPanningHandler.prototype.hideSubmenu=function(parent)
{
...
this.shape=new mxPolyline(new Array(),'#6482B9');
this.shape.isDashed=true;
this.shape.dialect=(this.graph.dialect!=mxConstants.DIALECT_SVG)?mxConstants.DIALECT_VML:mxConstants.DIALECT_SVG;
this.shape.init(this.graph.getView().getOverlayPane());
this.shape.node.style.display='none';
...
mxEvent.redirectMouseEvents(this.selectionBorder.node,this.graph,this.state.cell);
}
else
{
...
{
this.sizers.push(this.createSizer('default',this.LABEL_INDEX,(this.graph.dialect==mxConstants.DIALECT_SVG)?4:6,'yellow'));
}
}
else if(this.graph.isMovable(this.state.cell)&&!this.graph.isSizable(this.state.cell)&&this.state.width<2&&this.state.height<2)
{
this.sizers.push(this.createSizer('move',null,null,'yellow'));
}
}
};
...
var sizer=new mxRectangleShape(bounds,color||'#00FF00','black');
if(this.graph.dialect==mxConstants.DIALECT_SVG)
{
...
origin.x-=offset.x;
origin.y-=offset.y;
this.moveSizerTo(this.sizers[8],this.graph.snap(evt.clientX+origin.x),this.graph.snap(evt.clientY+origin.y));
mxEvent.consume(evt);
}
...
this.labelShape=new mxRectangleShape(new mxRectangle(),'yellow','black');
...
{
var bend=new mxRectangleShape(new mxRectangle(),'#00FF00','black');
this.initBend(bend);
...
{
var edge=this.connect(edge,this.marker.getValidState().cell,this.isSource,evt.ctrlKey&&this.cloneEnabled&&this.graph.isCloneable());
}
...
var color=(movable)?((connected)?'#0000FF':'#00FF00'):'#FF0000';
...
color=(movable)?((connected)?'#0000FF':'#00FF00'):'#FF0000';
...
this.points.push(new mxPoint(0,0));
bend=new mxRectangleShape(new mxRectangle(),'#00FF00','black');
this.initBend(bend);
bend.node.style.cursor='all-scroll';
this.installListeners(bend.node,bends.length);
bends.push(bend);
