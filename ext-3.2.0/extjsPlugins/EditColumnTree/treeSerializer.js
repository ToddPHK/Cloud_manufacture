 ...
 * 
 * @param {Function}
 *            (optional) A function, which when passed the node, returns true or
 *            false to include or exclude the node.
 * @param {Function}
 *            (optional) A function, which when passed an attribute name, and an
 *            attribute value, returns true or false to include or exclude the
 *            attribute.
 * @return {String}
 */
Ext.tree.TreePanel.prototype.toJsonString = function(nodeFilter,
		attributeFilter, attributeMapping) {
 ...
 * @return {String}
 */
Ext.tree.TreeNode.prototype.toJsonString = function(nodeFilter,
		attributeFilter, attributeMapping) {
	...
	for (var key in this.attributes) {
		if ((key != 'id')
				&& (!attributeFilter || attributeFilter(key,
						this.attributes[key]))) {
			if (c)
				result += ',';
 ...
 * @return {String}
 */
Ext.tree.TreeNode.prototype.toXmlString = function(nodeFilter, attributeFilter,
		attributeMapping) {
	// Exclude nodes based on caller-supplied filtering function
	if (nodeFilter && (nodeFilter(this) == false)) {
		return '';
